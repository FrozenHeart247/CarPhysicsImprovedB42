...
public final class LegacyPhysics {
    ...
    public static Output step(Spec spec, Conditions conditions, Settings settings, Input input, State state,
            double deltaSeconds) {
        return step(spec, conditions, settings, input, state, deltaSeconds, 1.0, spec.steeringClampRadians());
    }

    /** Dedicated drift changes steering response, not drivetrain or final angle amplification. */
    public static Output step(Spec spec, Conditions conditions, Settings settings, Input input, State state,
            double deltaSeconds, double steeringResponseMultiplier, double steeringClampRadians) {
        ...
        steeringFactor *= steeringAuthority * clamp(steeringResponseMultiplier, 1.0, 2.0);
        ...
        double steeringLimit = Math.min(spec.steeringClampRadians,
                clamp(steeringClampRadians, 0.05, 1.5)) * steeringAuthority;
