...
final class PzLegacyAccess {
    ...
    private final Method gameTimeRealDelta;
    ...
    PzLegacyAccess() throws ReflectiveOperationException {
        ...
        gameTimeRealDelta = method(gameTimeClass, "getRealworldSecondsSinceLastUpdate");
    ...
    double realDeltaSeconds() throws ReflectiveOperationException {
        Object time = invoke(gameTimeInstance, null);
        return time == null ? 1.0 / 60.0
                : clamp(((Number) invoke(gameTimeRealDelta, time)).doubleValue(), 1.0 / 240.0, .05);
    }

    double nativeMass(Object vehicle) throws ReflectiveOperationException {
    ...
    void applyWheelFrictionScale(Object vehicle, double requestedScale) throws ReflectiveOperationException {
        applyWheelFrictionScale(vehicle, clamp(requestedScale, 0.12, 1.0), Double.POSITIVE_INFINITY);
    }

    void applyKeyDriftFriction(Object vehicle, double grip, double driftGrip) throws ReflectiveOperationException {
        // Equivalent to min(1.8, baseFriction * currentGrip) * DriftGrip.
        // Keep the existing script owner/restoration path, but do not weaken
        // weather and tire losses by choosing min(environment, drift) instead of multiplying.
        applyWheelFrictionScale(vehicle, clamp(grip, 0, 1.8) * driftGrip, 1.8 * driftGrip);
    }

    double wheelFriction(Object vehicle) throws ReflectiveOperationException {
        return scriptWheelFriction.getFloat(script(vehicle));
    }

    private void applyWheelFrictionScale(Object vehicle, double scale, double ceiling) throws ReflectiveOperationException {
        Object script = script(vehicle);
        synchronized (scriptFrictionStates) {
            ScriptFrictionState state = scriptFrictionStates.get(script);
            if (scale >= 0.999 && ceiling == Double.POSITIVE_INFINITY) {
