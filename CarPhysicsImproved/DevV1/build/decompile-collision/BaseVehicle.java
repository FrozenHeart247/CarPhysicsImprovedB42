...
import zombie.inventory.ItemPickerJava.KeyNamer;
...
import zombie.vehicles.BaseVehicle.PositionHistoryEntry;
import zombie.vehicles.BaseVehicle.QuaternionfObjectPool;
import zombie.vehicles.BaseVehicle.TransformPool;
import zombie.vehicles.BaseVehicle.Vector2fObjectPool;
import zombie.vehicles.BaseVehicle.Vector3ObjectPool;
import zombie.vehicles.BaseVehicle.Vector3fObjectPool;
import zombie.vehicles.BaseVehicle.Vector4fObjectPool;
import zombie.vehicles.BaseVehicle.VehicleImpulse;
import zombie.vehicles.BaseVehicle.WeightedVehiclePart;
...
public final class BaseVehicle
   ...
   private final HitVars hitVars = new HitVars();
   ...
   public static final ThreadLocal<Vector3ObjectPool> TL_vector3_pool = ThreadLocal.withInitial(Vector3ObjectPool::new);
   ...
   private static void LoadVehicleTextures(Skin skin) {
      skin.textureData = LoadVehicleTexture(skin.texture);
   ...
   public boolean shouldAnimRecorderBeActive() {
      return super.shouldAnimRecorderBeActive() || (Boolean)PZOptional.ifPresent(this.getPVPPlayerDriver(), false, IsoMovingObject::shouldAnimRecorderBeActive);
   }
   ...
   private InventoryContainer tryCreateKeyRing() {
      String keyRingType = Container.KEY_RING.toString();
   ...
   public void putKeyToContainerServer(InventoryItem item, IsoGridSquare sq, IsoObject obj) {
      if (GameServer.server) {
         INetworkPacket.sendToRelative(PacketType.AddInventoryItemToContainer, obj.square.x, obj.square.y, new Object[]{this.container, item});
      }
   }
   ...
   public void updateSkin() {
      if (this.sprite != null && this.sprite.modelSlot != null && this.sprite.modelSlot.model != null) {
         ...
         Skin skin = this.script.getTextures();
         ...
         if (inst.tex != null) {
            ...
            DebugType.Animation.error("texture not found:", new Object[]{this.getSkin()});
         }
      }
   }
   ...
   public Texture getShadowTexture() {
      if (this.getScript() != null) {
         Skin skin = this.getScript().getTextures();
   ...
   public boolean isEngineSounding() {
      ...
      boolean isStartingWithCombinedSound = this.getEngineState() == engineStateTypes.StartingSuccess
         && this.getVehicleSounds().isCombinedWithEngineSound(this.getVehicleSounds().getEngineStartSound());
      return isStartingWithCombinedSound ? true : this.getEngineState() == engineStateTypes.Running;
   }
      ...
      if (this.script != null) {
         for (int i = 0; i < this.passengers.length; i++) {
            ...
            if (passenger != null && passenger.character != null) {
               Position pp = this.getPassengerPosition(i, "inside");
               if (pp != null) {
   ...
   public ModelInfo setModelVisible(VehiclePart part, Model scriptModel, boolean visible) {
      for (int i = 0; i < this.models.size(); i++) {
         ModelInfo info = this.models.get(i);
         if (info.part == part && info.scriptModel == scriptModel) {
            ...
            if (info.animPlayer != null) {
               info.animPlayer = (AnimationPlayer)Pool.tryRelease(info.animPlayer);
            }
   ...
   private String getModelScriptNameForPart(VehiclePart part, Model scriptModel) {
   ...
   private zombie.scripting.objects.VehicleScript.Passenger getScriptPassenger(int seat) {
      if (this.getScript() == null) {
         return null;
      } else {
   ...
   public Vector3f getPassengerPositionWorldPos(Position posn, Vector3f out) {
   ...
   public Anim getPassengerAnim(int seat, String id) {
      zombie.scripting.objects.VehicleScript.Passenger pngr = this.getScriptPassenger(seat);
      if (pngr == null) {
         return null;
      }

      for (int i = 0; i < pngr.anims.size(); i++) {
         Anim anim = (Anim)pngr.anims.get(i);
   ...
   public Position getPassengerPosition(int seat, String id) {
      zombie.scripting.objects.VehicleScript.Passenger pngr = this.getScriptPassenger(seat);
   ...
   public void playPassengerAnim(int seat, String animId, IsoGameCharacter chr) {
      if (chr != null) {
         Anim anim = this.getPassengerAnim(seat, animId);
         if (anim != null) {
   ...
   public void playPartAnim(VehiclePart part, String animId) {
      if (this.parts.contains(part)) {
         Anim anim = part.getAnimById(animId);
   ...
   public void playActorAnim(VehiclePart part, String animId, IsoGameCharacter chr) {
      if (chr != null) {
         if (this.parts.contains(part)) {
            Anim anim = part.getAnimById("Actor" + animId);
            if (anim != null) {
   ...
   private void playCharacterAnim(IsoGameCharacter chr, Anim anim, boolean snapDirection) {
   ...
   public void setCharacterPosition(IsoGameCharacter chr, int seat, String positionId) {
      zombie.scripting.objects.VehicleScript.Passenger pngr = this.getScriptPassenger(seat);
      if (pngr != null) {
         Position position = pngr.getPositionById(positionId);
         if (position != null) {
            if (this.getCharacter(seat) == chr) {
               ...
               if (position.area == null) {
                  ...
                  Area area = this.script.getAreaById(position.area);
                  Vector2 vector2 = (Vector2)Vector2ObjectPool.get().alloc();
   ...
   public void transmitCharacterPosition(int seat, String positionId) {
      if (GameClient.client) {
         INetworkPacket.send(PacketType.VehiclePassengerPosition, new Object[]{this, seat, positionId});
      }
   }
   ...
   public SwitchSeat getPassengerSwitchSeat(int seat, int index) {
      zombie.scripting.objects.VehicleScript.Passenger pngr = this.getScriptPassenger(seat);
      if (pngr == null) {
         return null;
      } else {
         return index >= 0 && index < pngr.switchSeats.size() ? (SwitchSeat)pngr.switchSeats.get(index) : null;
      }
   }

   private SwitchSeat getSwitchSeat(int seatFrom, int seatTo) {
      zombie.scripting.objects.VehicleScript.Passenger pngr = this.getScriptPassenger(seatFrom);
      if (pngr == null) {
         return null;
      }

      for (int i = 0; i < pngr.switchSeats.size(); i++) {
         SwitchSeat switchSeat = (SwitchSeat)pngr.switchSeats.get(i);
   ...
   public String getSwitchSeatAnimName(int seatFrom, int seatTo) {
      SwitchSeat switchSeat = this.getSwitchSeat(seatFrom, seatTo);
   ...
   public void switchSeat(IsoGameCharacter chr, int seatTo) {
      ...
      if (seatFrom != -1) {
         ...
         Position posInside = this.getPassengerPosition(seatTo, "inside");
         if (posInside == null) {
            Vector3f v = (Vector3f)TL_vector3f_pool.get().alloc();
   ...
   public float getEnterSeatDistance(int seat, float x, float y) {
      Position posOutside = this.getPassengerPosition(seat, "outside");
      if (posOutside != null) {
      ...
      for (int i = 0; i < this.parts.size(); i++) {
         ...
         if (part.getArea() != null && this.isInArea(part.getArea(), chr)) {
            ...
            if (func != null && !func.equals("")) {
               Area area = script.getAreaById(part.getArea());
               if (area != null) {
   ...
   private @Nullable VehiclePart getClosestWindow(float chrX, float chrY, float chrZ, float forwardDirectionX, float forwardDirectionY) {
      ...
      for (int i = 0; i < this.parts.size(); i++) {
         ...
         if (part.getWindow() != null && part.getArea() != null && this.isInArea(part.getArea(), chrX, chrY)) {
            Area area = this.script.getAreaById(part.getArea());
            if (area != null) {
   ...
   public boolean enter(int seat, IsoGameCharacter chr, Vector3f offset) {
      ...
      if (this.setPassenger(seat, chr, offset)) {
         ...
         if (GameClient.client) {
            INetworkPacket.send(PacketType.VehicleEnter, new Object[]{this, chr, seat});
         }
   ...
   public boolean enter(int seat, IsoGameCharacter chr) {
      if (this.getPartForSeatContainer(seat) != null && this.getPartForSeatContainer(seat).getInventoryItem() != null) {
         Position position = this.getPassengerPosition(seat, "outside");
   ...
   public boolean enterRSync(int seat, IsoGameCharacter chr, BaseVehicle v) {
      ...
      Position position = this.getPassengerPosition(seat, "inside");
      if (position != null) {
   ...
   public boolean exit(IsoGameCharacter chr) {
      ...
      if (this.clearPassenger(seat)) {
         ...
         if (GameClient.client) {
            INetworkPacket.send(PacketType.VehicleExit, new Object[]{this, chr, seat});
         }
               ...
               if (sq != null) {
                  if (bCollideWithObjects) {
                     for (int i = 0; i < sq.getObjects().size(); i++) {
                        IsoObject object = (IsoObject)sq.getObjects().get(i);
                  ...
                  if (bCollideWithCharacters) {
                     for (int i = 0; i < sq.getMovingObjects().size(); i++) {
                        IsoMovingObject object = (IsoMovingObject)sq.getMovingObjects().get(i);
                  ...
                  if (bCollideWithObjects) {
                     for (int i = 0; i < sq.getStaticMovingObjects().size(); i++) {
                        IsoMovingObject object = (IsoMovingObject)sq.getStaticMovingObjects().get(i);
      ...
      if (!this.removedFromWorld) {
         if (!this.getCell().vehicles.contains(this)) {
            ...
            if (!GameClient.client && this.getVehicleTowing() != null && this.getDriver() != null && this.getVehicleTowing().scriptName.equals("Base.Trailer")) {
               ...
               if (this.getCurrentSpeedKmHour() > 30.0F && trailer.getCondition() < 50.0F && !trailer.container.items.isEmpty()) {
                  ...
                  for (int i = 0; i < trailer.container.items.size(); i++) {
                     if (((InventoryItem)trailer.container.items.get(i)).getWeight() >= 3.5F) {
                        heavyItems.add((InventoryItem)trailer.container.items.get(i));
                     }
                  }
            ...
            if (this.physics != null) {
               ...
               if (this.jniIsCollide && this.limitCrash.Check()) {
                  ...
                  if (currentVelocity.lengthSquared() > this.lastLinearVelocity.lengthSquared() && delta > 6.0F) {
                     DebugType.Vehicle.trace("Vehicle vid=%d got sharp speed increase delta=%f", new Object[]{this.vehicleId, delta});
                     delta = 6.0F;
                  }
                  ...
                  if (delta > 1.0F) {
                     ...
                     DebugType.Vehicle.trace("Vehicle vid=%d crash delta=%f", new Object[]{this.vehicleId, delta});
                     Vector3f forward = this.getForwardVector(allocVector3f());
            ...
            for (int i = 0; i < this.impulsesFromSquishedBodies.length; i++) {
               VehicleImpulse impulse = this.impulsesFromSquishedBodies[i];
               if (impulse != null) {
                  impulse.enable = false;
               }
            }
            ...
            if (GameClient.client) {
               GameClient.instance
                  .sendClientCommandV(
                     ...
                     "vehicle",
                     "damageFromHitChr",
                     new Object[]{"vehicle", this.getId(), "dmgFront", totalDmgFront, "dmgBack", totalDmgBack}
                  );
               return;
            }
   ...
   public void damageFromHitChr(int dmgFront, int dmgBack) {
      if (dmgFront > 0) {
         DebugType.VehicleHit.debugln("Vehicle receives damage from pedestrian hits on the Front: %d", new Object[]{dmgFront});
         this.addDamageFrontHitAChr(dmgFront);
      }

      if (dmgBack > 0) {
         DebugType.VehicleHit.debugln("Vehicle receives damage from pedestrian hits on the Back: %d", new Object[]{dmgBack});
   ...
   private void updateAnimationPlayer(AnimationPlayer animPlayer, VehiclePart part) {
      if (animPlayer != null && animPlayer.isReady()) {
         ...
         for (int i = 0; i < multiTrack.getTrackCount(); i++) {
            AnimationTrack track = (AnimationTrack)multiTrack.getTracks().get(i);
   ...
   protected void updateTransform() {
      if (this.sprite.modelSlot != null) {
         ...
         modelRotQ.rotationXYZ(modelRotate.x * (float) (Math.PI / 180.0), modelRotate.y * (float) (Math.PI / 180.0), modelRotate.z * (float) (Math.PI / 180.0));
         ...
         for (int i = 0; i < this.models.size(); i++) {
            ModelInfo modelInfo = this.models.get(i);
            Model scriptModel = modelInfo.scriptModel;
            ...
            if (modelInfo.wheelIndex == -1) {
               if (modelInfo.part != null
                  ...
                  zombie.core.skinnedmodel.model.Model parentModel = parentModelInfo.modelInstance.model;
               ...
               Wheel scriptWheel = this.getScript().getWheel(modelInfo.wheelIndex);
               VehicleImpulse impulse = modelInfo.wheelIndex < this.impulsesFromSquishedBodies.length
   ...
   public void updatePhysicsNetwork() {
      ...
      if (update) {
         INetworkPacket.send(this.isReliable ? PacketType.VehiclePhysicsReliable : PacketType.VehiclePhysicsUnreliable, new Object[]{this});
   ...
   public void checkPhysicsValidWithServer() {
      ...
      if (this.limitPhysicValid.Check() && Bullet.getOwnVehiclePhysics(this.vehicleId, physicsParams) == 0) {
         ...
         if (diffX > 0.05F || diffY > 0.05F) {
            VehicleManager.instance.sendVehicleRequest(this.vehicleId, (short)2);
            DebugType.Vehicle.trace("diff-x=%f diff-y=%f delta=%f", new Object[]{diffX, diffY, 0.05F});
         }
      }
   }
   ...
   public void crash(float delta, boolean front) {
      ...
      if (GameClient.client) {
         ...
         GameClient.instance.sendClientCommandV(null, "vehicle", "crash", new Object[]{"vehicle", this.getId(), "amount", delta, "front", front});
      } else {
         float modifier = 1.3F;
         float dmg = delta;
         ...
         IsoPlayer driver = (IsoPlayer)Type.tryCastTo(this.getDriverRegardlessOfTow(), IsoPlayer.class);
   ...
   public int testCollisionWithProneCharacter(IsoMovingObject chr, float angleX, float angleY, boolean doSound, Vector2 outImpactPosOnVehicle) {
      ...
      for (int i = 0; i < this.script.getWheelCount(); i++) {
         Wheel scriptWheel = this.script.getWheel(i);
   ...
   public void applyImpulseGeneric(float fromX, float fromY, float fromZ, float impulseDirX, float impulseDirY, float impulseDirZ, float impulseStrength) {
      ...
      DebugType.General
         .println(
            ...
            new Object[]{
               fromX, fromY, fromZ, impulse.relPos.x, impulse.relPos.y, impulse.relPos.z, this.isActive ? "true" : "false", this.isStatic ? "true" : "false"
            }
         );
   }
   ...
   public boolean isExitBlocked(int seat) {
      Position posInside = this.getPassengerPosition(seat, "inside");
      Position posOutside = this.getPassengerPosition(seat, "outside");
      if (posInside != null && posOutside != null) {
         ...
         if (posOutside.area != null) {
            Vector2 vector2 = (Vector2)Vector2ObjectPool.get().alloc();
            Area area = this.script.getAreaById(posOutside.area);
            Vector2 areaPos = this.areaPositionWorld4PlayerInteract(area, vector2);
            exitPos.x = areaPos.x;
            exitPos.y = areaPos.y;
            Vector2ObjectPool.get().release(vector2);
   ...
   public boolean isPassengerUseDoor2(IsoGameCharacter chr, int seat) {
      Position posn = this.getPassengerPosition(seat, "outside2");
      if (posn != null) {
   ...
   public boolean isExitBlocked2(int seat) {
      Position posInside = this.getPassengerPosition(seat, "inside");
      Position posOutside = this.getPassengerPosition(seat, "outside2");
      if (posInside != null && posOutside != null) {
         Vector3f exitPos = this.getPassengerPositionWorldPos(posOutside, (Vector3f)TL_vector3f_pool.get().alloc());
         exitPos.z = 0.0F;
         Vector3f seatedPos = this.getPassengerPositionWorldPos(posInside, (Vector3f)TL_vector3f_pool.get().alloc());
   ...
   private Vector2 areaPositionLocal(Area area) {
      return this.areaPositionLocal(area, new Vector2());
   }

   private Vector2 areaPositionLocal(Area area, Vector2 out) {
   ...
   public Vector2 areaPositionWorld(Area area) {
      return this.areaPositionWorld(area, new Vector2());
   }

   public Vector2 areaPositionWorld(Area area, Vector2 out) {
      if (area == null) {
         return null;
      }
   ...
   public Vector2 areaPositionWorld4PlayerInteract(Area area) {
      return this.areaPositionWorld4PlayerInteract(area, new Vector2());
   }

   public Vector2 areaPositionWorld4PlayerInteract(Area area, Vector2 out) {
      Vector3f ext = this.script.getExtents();
      Vector3f com = this.script.getCenterOfMassOffset();
      ...
      if (this.getScript() != null) {
         ...
         for (int i = 0; i < this.parts.size(); i++) {
            ...
            if (part.getArea() != null) {
               Area area = this.getScript().getAreaById(part.getArea());
               if (area != null) {
                  ...
                  if (center != null) {
                     ...
                     this.getController()
                        .drawRect(
                           forward, center.x - WorldSimulation.instance.offsetX, center.y - WorldSimulation.instance.offsetY, 0.1F, 0.1F, 1.0F, 0.0F, 0.0F
                        );
                  }
               }
            }
   ...
   private void renderInterpolateBuffer_drawVertLine(long x1, Color col, float x, float y, float w, float h, long start, long end, boolean drawParity) {
      ...
      TextManager.instance
         .DrawString(
            tempX + x, y + h + (drawParity ? 0.0F : 10.0F), String.format("%.1f", (float)(x1 - x1 / 100000L * 100000L) / 1000.0F), col.r, col.g, col.b, col.a
         );
   }
   ...
   private void renderAuthorizations() {
      ...
      IsoPlayer owner = (IsoPlayer)GameClient.IDToPlayerMap.get(this.netPlayerId);
            ...
            if (part != null) {
               ...
               if (area != null) {
                  ...
                  if (center == null) {
                     ...
                     this.getController()
                        .drawRect(
                           forward, center.x - WorldSimulation.instance.offsetX, center.y - WorldSimulation.instance.offsetY, area.w, area.h / 2.0F, r, g, b
                        );
   ...
   private void renderVelocity() {
      ...
      TextManager.instance
         .DrawStringCentre(
            sx,
            sy,
            "velocity %.3f,%.3f,%.3f speed2D %.3f".formatted(this.jniLinearVelocity.x, this.jniLinearVelocity.y, this.jniLinearVelocity.z, this.getSpeed2D()),
            1.0,
            1.0,
            1.0,
            1.0
   ...
   public void resumeRunningAfterLoad() {
      if (GameClient.client) {
         ...
         if (driver != null) {
            ...
            GameClient.instance.sendClientCommandV((IsoPlayer)this.getDriver(), "vehicle", "startEngine", new Object[]{"haveKey", haveKey});
         }
   ...
   public void addToWorld(boolean crashed) {
      if (this.addedToWorld) {
         ...
         if (this.chunk != null
            && this.chunk.jobType != JobType.SoftReset
   ...
   public void removeFromWorld() {
      ...
      DebugType.Vehicle.trace("BaseVehicle.removeFromWorld() %s id=%d", new Object[]{this, this.vehicleId});
      if (!this.removedFromWorld) {
         if (!this.addedToWorld) {
            DebugType.Vehicle.debugln("ERROR: removing vehicle but addedToWorld=false %s id=%d", new Object[]{this, this.vehicleId});
         }
   ...
   public <T> PZArrayList<ItemContainer> getVehicleItemContainers(T paramToCompare, ICallback<T, ItemContainer> isValidPredicate) {
   ...
   private boolean isInArea(String areaId, float chrX, float chrY) {
      if (areaId != null && this.getScript() != null) {
         Area area = this.getScript().getAreaById(areaId);
         if (area == null) {
            return false;
         } else {
            Vector2 vector2 = (Vector2)Vector2ObjectPool.get().alloc();
   ...
   public boolean canAccessContainer(int partIndex, IsoGameCharacter chr) {
      ...
      if (part == null) {
         ...
         Part scriptPart = part.getScriptPart();
         if (scriptPart == null) {
            return false;
   ...
   public void tryHotwire(int electricityLevel) {
      ...
      if (GameServer.server) {
         GlobalObject.playServerSound(sound, this.square);
   ...
   public IsoPlayer getPVPPlayerDriver() {
      if (GameClient.client || GameServer.server) {
         IsoPlayer netDriver = (IsoPlayer)GameServer.IDToPlayerMap.get(this.getNetPlayerId());
         if (netDriver != null) {
            return netDriver;
         }
      }

      return (IsoPlayer)Type.tryCastTo(this.getDriverRegardlessOfTow(), IsoPlayer.class);
   }
            ...
            if (!this.isHotwired()) {
               if (!GameServer.server && keysOnContact && !this.isKeysInIgnition()) {
                  ...
                  if (key != null) {
                     ...
                     if (GameClient.client) {
                        GameClient.instance
                           .sendClientCommandV((IsoPlayer)this.getDriver(), "vehicle", "putKeyInIgnition", new Object[]{"key", key, "container", containerID});
                     }
               ...
               if (!keysOnContact && this.isKeysInIgnition() && !GameServer.server) {
                  ...
                  InventoryItem key = (InventoryItem)this.ignitionSwitch.getItems().get(0);
   ...
   public void putKeyInIgnition(InventoryItem key, int containerID) {
      if (GameServer.server) {
         if (key instanceof zombie.inventory.types.Key) {
   ...
   private void randomizeContainers() {
      if (!GameClient.client) {
         ...
         VehicleDistribution distrib = (VehicleDistribution)ItemPickerJava.VehicleDistributions.get(scriptType + this.getSkinIndex());
         if (distrib != null) {
            doSpecific = false;
         } else {
            distrib = (VehicleDistribution)ItemPickerJava.VehicleDistributions.get(scriptType);
         }
         ...
         if (distrib == null) {
            ...
            ItemPickerRoom contDistrib;
            if (doSpecific && Rand.Next(100) <= specialLootChance && !distrib.specific.isEmpty()) {
               contDistrib = (ItemPickerRoom)PZArrayUtil.pickRandom(distrib.specific);
            } else {
            ...
            if (!StringUtils.isNullOrWhitespace(this.specificDistributionId)) {
               for (int i = 0; i < distrib.specific.size(); i++) {
                  ItemPickerRoom room = (ItemPickerRoom)distrib.specific.get(i);
   ...
   private void randomizeContainers(ItemPickerRoom contDistrib) {
      for (int i = 0; i < this.parts.size(); i++) {
         VehiclePart part = this.parts.get(i);
         if (part.getItemContainer() != null) {
            if (GameServer.server && GameServer.softReset) {
   ...
   private void randomizeContainer(VehiclePart part, ItemPickerRoom contDistrib) {
      if (!GameClient.client) {
         if (contDistrib != null) {
   ...
   public void onVehicleAlarmEvent(VehicleAlarmEvent event) {
      switch (1.$SwitchMap$zombie$vehicles$VehicleAlarmEvent[event.ordinal()]) {
         case 1:
            if (GameServer.server) {
            ...
            if (this.script.getSounds().alarmEnable) {
               IsoGameCharacter driver = this.getDriver();
               IsoPlayer player = (IsoPlayer)Type.tryCastTo(driver, IsoPlayer.class);
   ...
   public void updateTotalMass() {
      ...
      for (int i = 0; i < this.parts.size(); i++) {
         ...
         if (part.getInventoryItem() != null) {
            plusMass += part.getInventoryItem().getWeight();
         }
      }
      ...
      for (int i = 0; i < this.getPartCount(); i++) {
         ...
         if (part.getInventoryItem() != null) {
            if (part.getInventoryItem().getBrakeForce() > 0.0F) {
               float newBrakeForce = VehiclePart.getNumberByCondition(part.getInventoryItem().getBrakeForce(), part.getInventoryItem().getCondition(), 5.0F);
            ...
            if (part.getInventoryItem().getWheelFriction() > 0.0F) {
               ...
               float friction = VehiclePart.getNumberByCondition(part.getInventoryItem().getWheelFriction(), part.getInventoryItem().getCondition(), 0.2F);
            ...
            if (part.getInventoryItem().getDurability() > 0.0F) {
               part.setDurability(part.getInventoryItem().getDurability());
            }
         }
      }
   ...
   private void updateBulletStatsWheel(int wheelIndex, float[] data, Vector3f worldPos, float frictionMul, int chanceofbump, double period, double susp) {
      ...
      Wheel scriptWheel = this.script.getWheel(wheelIndex);
               ...
               if (this.getPassenger(i).character != null) {
                  ...
                  if (!chr.isGodMod() && !GameClient.client) {
                     ...
                     if (GameServer.server && chr instanceof IsoPlayer player) {
                        INetworkPacket.send(player, PacketType.PlayerDamage, new Object[]{player});
                     }
                  }
               }
            }
   ...
   public boolean isAnyTireMissing() {
      ...
      for (int i = 0; i < script.getWheelCount(); i++) {
         Wheel scriptWheel = script.getWheel(i);
         VehiclePart part = this.getPartById("Tire" + scriptWheel.getId());
         if (part == null || part.getInventoryItem() == null) {
            return true;
         }
   ...
   public InventoryItem getCurrentKey() {
      return !this.ignitionSwitch.getItems().isEmpty() ? (InventoryItem)this.ignitionSwitch.getItems().get(0) : null;
   }
   ...
   public boolean shouldNotHaveLoot() {
      return this.getSquare() != null
            ...
            && Objects.equals(
               Objects.requireNonNull(IsoWorld.instance.metaGrid.getVehicleZoneAt(this.getSquare().getX(), this.getSquare().getY(), this.getSquare().getZ())).name,
               "junkyard"
            )
         ? true
         : this.getSquare() != null
   ...
   public AnimationPlayer getAnimationPlayer() {
      ...
      zombie.core.skinnedmodel.model.Model model = ModelManager.instance.getLoadedModel(modelName);
   ...
   public void addPointConstraint(IsoPlayer player, BaseVehicle vehicleB, String attachmentA, String attachmentB, Boolean remote) {
      ...
      if (v1 != null && v2 != null) {
         ...
         if (GameServer.server
            && player != null
            && vehicleA.netPlayerAuthorization == Authorization.Server
            && vehicleB.netPlayerAuthorization == Authorization.Server) {
   ...
   public boolean breakConstraintOnServer() {
      ...
      if (vehicleB != null && vehicleB.isLocalPhysicSim()) {
         ...
         GameClient.instance.sendClientCommandV(player, "vehicle", "detachTrailerSpontaneous", new Object[]{"vehicle", vehicleA.getId()});
         return true;
      } else {
         return false;
      }
   ...
   public VehicleEngineRPM getVehicleEngineRPM() {
      if (this.vehicleEngineRpm == null) {
         ...
         if (this.vehicleEngineRpm == null) {
            DebugType.Vehicle.warn("unknown vehicleEngineRPM \"%s\"", new Object[]{this.getScript().getEngineRPMType()});
   ...
   public void onHitLandmine(IsoGridSquare square) {
      ...
      DebugType.General
         .println(
            ...
            new Object[]{(float)square.getX(), (float)square.getY(), (float)square.getZ(), this.isActive ? "true" : "false", this.isStatic ? "true" : "false"}
         );
   }

   public void onJump() {
      this.applyImpulseGeneric(this.getX(), this.getY(), this.getZ(), 0.0F, 0.0F, 1.0F, this.getFudgedMass() * 500.0F);
      DebugType.General.println("Jump! isActive: %s, isStatic: %s", new Object[]{this.isActive ? "true" : "false", this.isStatic ? "true" : "false"});
   }
   ...
   public void addAnimalFromHandsInTrailer(IsoAnimal animal, IsoPlayer player) {
      ...
      if (item != null) {
         ...
         DebugType.Animal.error("Animal not found: id=%d/%d", new Object[]{animal.getAnimalID(), animal.getOnlineID()});
      }
   ...
   public static void keyNamerVehicle(InventoryItem item, BaseVehicle vehicle) {
      if (item != null && vehicle != null) {
         ...
         if (!item.getType().equals("KeyRing") && !item.hasTag(ItemTag.KEY_RING)) {
            ...
            item.setName(
               Translator.getText(item.getScriptItem().getDisplayName(), new Object[0])
                  + " - "
                  + Translator.getText("IGUI_VehicleName" + carName, new Object[0])
            );
         }
      }
   }
               ...
               if (!outfitName.contains("Young") && !outfitName.contains("Student") || vehicleType.contains("Small") && Rand.Next(2) == 0) {
                  ...
                  if (!vehicleType.contains("Small") && !vehicleType.contains("StationWagon")
                     ...
                     if (!vehicleType.contains("Pickup") && !vehicleType.contains("Van")
                        || !outfitName.contains("Classy") && !outfitName.contains("Doctor") && !outfitName.contains("Golfer") && !outfitName.contains("Trader")
                        )
                      {
   ...
   public void forceVehicleDistribution(String distribution) {
      VehicleDistribution distro = (VehicleDistribution)ItemPickerJava.VehicleDistributions.get(distribution);
      ItemPickerRoom distro2 = distro.normal;
   ...
   private void checkVehicleFailsToStartWithZombiesTargeting() {
      if (!GameServer.server) {
         for (int i = 0; i < this.getMaxPassengers(); i++) {
            Passenger passenger = this.getPassenger(i);
            IsoPlayer player = (IsoPlayer)Type.tryCastTo(passenger.character, IsoPlayer.class);
            if (player != null && player.isLocalPlayer()) {
   ...
   public boolean leftSideFuel() {
      Area area = this.getScript().getAreaById("GasTank");
   ...
   public VehiclePart getNearestVehiclePart(float x, float y, float z, boolean useDestroyed) {
      ...
      for (int i = 0; i < this.script.getAreaCount(); i++) {
         Area area = this.script.getArea(i);
      ...
      if (vehiclePart != null) {
         ...
         DebugType.Combat
            .debugln(
               ...
               new Object[]{vehiclePart.getId(), vehiclePart.getDurability(), damage, calculatedDamage}
            );
      }
   }
   ...
   private VehiclePart getWeightedRandomSidePart(String side) {
      ArrayList<WeightedVehiclePart> activeVehiclePartList = new ArrayList<>();
   ...
   public float getSpecialKeyRingChance() {
      return !this.getScript().hasSpecialKeyRing() ? this.keySpawnChancedD100 : Math.max(this.keySpawnChancedD100, this.getScript().getSpecialKeyRingChance());
   }
