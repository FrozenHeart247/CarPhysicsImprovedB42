...
import dev.carphysicsimproved.v1.runtime.LegacyDriftKey;
import dev.carphysicsimproved.v1.runtime.LegacyDrivingKeys;
...
public final class CarPhysicsImprovedV1Mod {
    ...
    private static volatile LegacyDrivingKeys drivingKeys = LegacyDrivingKeys.defaults();
    private static volatile boolean rearAxleDrift;
    ...
    public static void configureDriftKey(int key, boolean shift, boolean ctrl, boolean alt) {
        LegacyDrivingKeys current = drivingKeys;
        drivingKeys = new LegacyDrivingKeys(new LegacyDriftKey(key, shift, ctrl, alt),
                current.shiftUp(), current.shiftDown());
    }

    public static void configureShiftKeys(int up, boolean upShift, boolean upCtrl, boolean upAlt,
            int down, boolean downShift, boolean downCtrl, boolean downAlt) {
        drivingKeys = new LegacyDrivingKeys(drivingKeys.drift(),
                new LegacyDriftKey(up, upShift, upCtrl, upAlt),
                new LegacyDriftKey(down, downShift, downCtrl, downAlt));
    }

    public static LegacyDrivingKeys drivingKeys() {
        return drivingKeys;
    }

    public static LegacyDriftKey driftKey() {
        return drivingKeys.drift();
    }

    public static void configureRearAxleDrift(boolean value, double rearGrip) {
        rearAxleDrift = value;
        rearAxleGrip = LegacyAxleDrift.grip(rearGrip);
        if (!value) LegacyAxleDriftHooks.restoreAll();
    }

    public static boolean rearAxleDrift() { return rearAxleDrift; }

    public static double rearAxleGrip() { return rearAxleGrip; }

    public static void configureKeyDrift(double rotation, double grip, double steeringBoost, double minimumSpeedKph) {
        keyDriftTuning = new LegacyKeyDrift.Tuning(rotation, grip, steeringBoost, minimumSpeedKph);
    }
