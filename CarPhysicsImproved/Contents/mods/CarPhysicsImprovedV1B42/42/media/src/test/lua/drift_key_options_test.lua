...
local function runCase(lines, key, savedMarks, sandbox)
    ...
    PZAPI = { ModOptions = {} }
    function PZAPI.ModOptions:create(_, title)
    ...
    function PZAPI.ModOptions:load()
        ...
        if savedMarks then
            options.dict.SkidMarkLifetime.value = savedMarks.lifetime
            options.dict.SkidMarkOpacity.value = savedMarks.opacity
        end
...
assert(calls.configureTireTracks[2] == 60 and calls.configureTireTracks[3] == .25,
    "Tire mark defaults must reach the Java renderer on boot")
...
assert(calls.configureTireTracks[2] == 20 and calls.configureTireTracks[3] == .8,
    "Defaults must not lock the sliders or override player-selected values")
...
print("drift_key_options_test: title, tire defaults, saved values, rebinding and gear combinations passed")
