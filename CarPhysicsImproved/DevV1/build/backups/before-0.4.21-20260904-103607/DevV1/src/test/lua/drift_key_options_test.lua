...
assert(calls.configureRearAxleDrift[1] == true and calls.configureRearAxleDrift[2] == .45,
    "Test build defaults to rear-only experiment with .45 grip")
options.dict.RearAxleDrift.value = false
options.apply()
assert(calls.configureRearAxleDrift[1] == false, "Checkbox must select legacy model")
